...
r = 5
Q = 0
for i in range(int(N/2) - int(L/h/2.0) - r, int(N/2) + int(L/h/2.0) + r):
	Q += Ey[i, int(N/2) + int(d/h/2.0) + r] * h
	Q -= Ey[i, int(N/2) + int(d/h/2.0) - r] * h
for i in range(int(N/2) + int(d/h/2.0) - r, int(N/2) + int(d/h/2.0) + r):
	Q -= Ex[int(N/2) - int(L/h/2.0) - r, i] * h
	Q += Ex[int(N/2) + int(L/h/2.0) + r, i] * h
Q *= epsilon
C_nonideal = Q / V0
print(C_nonideal)
...
print((C_nonideal - C_ideal) / C_ideal * 100)