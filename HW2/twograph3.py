...
chart2 = graph(width = 450, align = 'left')
...
dt = 0.0001
t = 0
Ek_t = gcurve(graph = chart, color = color.blue, width = 4)
Ug_t = gcurve(graph = chart, color = color.red, width = 4)
avEk_t = gcurve(graph = chart2, color = color.blue, width = 4)
avUg_t = gcurve(graph = chart2, color = color.red, width = 4)
avEk, avUg = 0, 0
while True:
    rate(5000)
    t += dt
    ...
    Ek, Ug = 0, 0
    for i in range(5):
        Ek += 0.5*m*balls[i].v.mag**2
        Ug += m*g.mag*(balls[i].pos.y+2)
    Ek_t.plot(pos = (t, Ek))
    Ug_t.plot(pos = (t, Ug))
    avEk = (avEk*(t-dt)+Ek*dt)/t
    avUg = (avUg*(t-dt)+Ug*dt)/t
    avEk_t.plot(pos = (t, avEk))
    avUg_t.plot(pos = (t, avUg))
